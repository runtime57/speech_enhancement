...
class EncoderGRNN(nn.Module):
    def __init__(self, params):
        super().__init__()
        p = params
        self.block_name = p.block_name
        ...
        self.erb_conv0 = make_block(
            self.block_name, 1, layer_width, p.nb_erb, k0, stride=1, groups=groups, lookahead=cl
        )
...
class DfNetConv(nn.Module):
    def __init__(
        ...
        valid_block_names = {"conv", "xconv", "dws", "xdws", "mb", "xmb", "mm", "xmm"}
        if block_name not in valid_block_names:
            raise ValueError(f"Unsupported block_name={block_name}. Expected one of {sorted(valid_block_names)}")
