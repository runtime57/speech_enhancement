...
from src.model.modules import LocalSnrTarget, Mask
from src.utils.config_utils import stft_config
from src.utils.df_utils import angle, as_complex
from src.utils.erb_utils import make_erb_fb_matrix_torch
...
def _resample_audio(audio: Tensor, orig_sr: int, new_sr: int) -> Tensor:
    if orig_sr == new_sr:
        return audio
    try:
        import torchaudio
    except Exception as exc:  # pragma: no cover
        raise ImportError("torchaudio is required for ASR loss resampling.") from exc
    return torchaudio.functional.resample(audio, orig_freq=orig_sr, new_freq=new_sr)
...
class MaskLoss(nn.Module):
    def __init__(
        ...
        mask: str = "iam",
        gamma: float = 0.6,
        powers: Optional[Iterable[int]] = None,
        factors: Optional[Iterable[float]] = None,
        f_under: float = 1,
        eps=1e-12,
        factor: float = 1.0,
        ...
        self.powers = list(powers) if powers is not None else [2]
        self.factors = list(factors) if factors is not None else [1.0]
...
class MaskSpecLoss(nn.Module):
    def __init__(
        ...
        super().__init__()
        self.f_max_idx = f_max_idx
        self.apply_mask = Mask(erb_inv_fb)
...
class Loss(nn.Module):
    """Wrapper aggregating all loss terms; returns a dict with key ``loss``."""

    def __init__(
        self,
        *,
        # Mask loss
        mask_factor: float = 0.0,
        ...
        # Spectral loss
        spec_factor_magnitude: float = 0.0,
        spec_factor_complex: float = 0.0,
        spec_factor_under: float = 1.0,
        spec_gamma: float = 1.0,
        # Multi-resolution spectral loss
        mr_factor: float = 0.0,
        mr_factor_complex: float = 0.0,
        mr_gamma: float = 1.0,
        mr_fft_sizes: Tuple[int, ...] = (512, 1024, 2048),
        # SDR losses
        sdr_factor: float = 0.0,
        sdr_segmental_ws: Tuple[int, ...] = (),
        ...
        asr_loss_lm: Literal["CTC", "CrossEntropy"] = "CrossEntropy",
        ...
        fb_fwd = make_erb_fb_matrix_torch(
            self.sr,
            self.fft_size,
            int(p.nb_erb),
            int(p.min_nb_freqs),
        ...
        fb_inv = make_erb_fb_matrix_torch(
            self.sr,
            self.fft_size,
            int(p.nb_erb),
            int(p.min_nb_freqs),
        ...
        self.lsnr_target = LocalSnrTarget(
            ws=20,
            target_snr_range=(float(p.lsnr_min) - 1.0, float(p.lsnr_max) + 1.0),
        )
        self.istft = istft
        self.summaries: Dict[str, List[Tensor]] = self.reset_summaries()

        # Mask loss
        self.ml_f = float(mask_factor)
        self.ml_mask = str(mask_type)
        self.ml_gamma = float(mask_gamma)
        self.ml_gamma_pred = float(mask_gamma_pred)
        self.ml_f_under = float(mask_f_under)
        if mask_max_freq <= 0:
            self.ml_f_max_idx = None
        else:
            self.ml_f_max_idx = int(mask_max_freq / (self.sr / self.fft_size))
        if self.ml_f > 0:
            if self.ml_mask == "spec":
                self.ml = MaskSpecLoss(
                    erb_inv_fb=fb_inv,
                    factor=self.ml_f,
                    gamma=self.ml_gamma,
                ...
                self.ml = MaskLoss(
                    ...
                    factors=list(mask_factors),
                    powers=list(mask_powers),
                    f_max_idx=self.ml_f_max_idx,
                )
        else:
            self.ml = None
        ...
        # Spectral loss
        self.sl_fm = float(spec_factor_magnitude)
        self.sl_fc = float(spec_factor_complex)
        self.sl_fu = float(spec_factor_under)
        self.sl_gamma = float(spec_gamma)
        ...
        # Multi-resolution spectral loss
        self.mrsl_f = float(mr_factor)
        self.mrsl_fc = float(mr_factor_complex)
        self.mrsl_gamma = float(mr_gamma)
        self.mrsl_ffts = list(mr_fft_sizes)
        self.mrsl = (
            MultiResSpecLoss(self.mrsl_ffts, self.mrsl_gamma, self.mrsl_f, self.mrsl_fc)
            if self.mrsl_f > 0
            else None
        )

        # SDR loss
        self.sdrl_f = float(sdr_factor)
        self.sdrl = None
        if self.sdrl_f > 0:
            if len(sdr_segmental_ws) > 0 and any(ws > 0 for ws in sdr_segmental_ws):
                self.sdrl = SegSdrLoss(list(sdr_segmental_ws), factor=self.sdrl_f, overlap=sdr_overlap)
            else:
        ...
        self.asrl_loss_lm = str(asr_loss_lm)
        self.asrl_m = str(asr_model)
        self.asrl = None
        if self.asrl_f > 0 or self.asrl_f_lm > 0:
            self.asrl = ASRLoss(
                ...
                loss_lm=self.asrl_loss_lm,  # type: ignore[arg-type]
                model=self.asrl_m,
            )

        need_td = (
            self.store_losses
            or self.mrsl is not None
            or self.sdrl is not None
    ...
    def _wave_to_stft(self, x: Tensor) -> Tensor:
        # x: [B,T] or [B,C,T]
        if x.dim() == 2:
            sh = (x.shape[0],)
        elif x.dim() == 3:
            ...
            raise ValueError(f"Expected waveform as [B,T] or [B,C,T], got {tuple(x.shape)}")
        t = x.shape[-1]
        ...
        out = out.view(*sh, *out.shape[-2:])  # [..., F, T]
        if out.dim() == 3:
            out = out.permute(0, 2, 1).unsqueeze(1)  # [B, 1, T, F]
        else:
            out = out.permute(0, 1, 3, 2)  # [B, C, T, F]
        return torch.view_as_real(out)

    def _as_spec(self, x: Tensor) -> Tensor:
        # Normalize input to packed real/imag STFT of shape [B, C, T, F, 2].
        if torch.is_complex(x):
            x = torch.view_as_real(x)
        if x.dim() <= 3:
            return self._wave_to_stft(x)
        if x.dim() == 4:
            if x.size(-1) != 2:
                raise ValueError(f"Expected packed STFT last dim=2, got {tuple(x.shape)}")
        ...
        if x.dim() == 5:
            if x.size(-1) != 2:
                raise ValueError(f"Expected packed STFT last dim=2, got {tuple(x.shape)}")
            if x.shape[2] == self.freq_bins and x.shape[3] != self.freq_bins:
                x = x.permute(0, 1, 3, 2, 4)
            return x
        raise ValueError(f"Unsupported STFT tensor shape: {tuple(x.shape)}")
    ...
    def forward(
        ...
        **batch,
        ...
        enh = enh if enh is not None else batch.get("enhanced", batch.get("enh"))
        if enh is None:
        ...
        if need_td:
            if self.istft is None:
                raise RuntimeError("Time-domain losses requested but `istft` is not available.")
            enhanced_td = self.istft(enh_spec)
        ...
        if self.ml is not None and self.ml_f != 0:
            if m is None:
                raise ValueError("Mask loss enabled but `m`/`mask` is missing.")
            m_in = m.unsqueeze(1) if m.dim() == 3 else m
            ml = self.ml(input=m_in, clean=clean_spec, noisy=noisy_spec)

        if self.sl is not None and self.sl_f != 0:
            sl = self.sl(input=enh_spec, target=clean_spec)

        if self.mrsl is not None and self.mrsl_f > 0:
            mrsl = self.mrsl(enhanced_td, clean_td)

        if self.asrl is not None and (self.asrl_f > 0 or self.asrl_f_lm > 0):
            asrl = self.asrl(enhanced_td, clean_td)

        if self.lsnrl is not None and self.lsnr_f != 0:
            if lsnr is None:
                raise ValueError("LocalSnrLoss enabled but `lsnr` is missing.")
