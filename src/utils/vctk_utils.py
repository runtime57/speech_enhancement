...
def get_flac_paths(directory='.'):
    start_path = Path(directory)
    flac_list = [
        str(file.absolute())
        for file in start_path.rglob('*.flac')
    ]
    return list(sorted(flac_list))
...
def create_vctk_split(split_name, rawdata_path, dataset_path):
    ...
    clean_paths = get_flac_paths(rawdata_path / f"clean_{split_name}set_wav")
    noisy_paths = get_flac_paths(rawdata_path / f"noisy_{split_name}set_wav")
